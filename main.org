...
Nonostante le buone /performance/ del modello (riassunte nella tabella [[dbscan_performance]]), si nota che il secondo /cluster/ $C_1$ contiene un numero non indifferente di immagini eterogenee.
Infatti l'algoritmo è riuscito a individuare un solo /cluster/ ($C_0$, che e' il cluster di elementi rigettati) ben definito, considerando la variabile risposta.
